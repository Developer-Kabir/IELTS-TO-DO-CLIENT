...
const Notes = () => {
    ...
    useEffect(() => {
        ...
        const url = `https://morning-forest-59623.herokuapp.com/user?email=${email}`;
    ...
    const removeNote = (id) => {
        ...
        if (decision) {
            const url = `https://morning-forest-59623.herokuapp.com/note/${id}`;
            fetch(url, {
                method: 'DELETE',
            })
                .then(res => res.json())
    ...
    const completeNote = id => {
        ...
        if (decision) {
            ...
            const url = `https://morning-forest-59623.herokuapp.com/updateDecoration/${id}`;
            fetch(url, {
                method: "PUT",
                headers: {
